...
Column {
    ...
    SettingsCard {
        ...
        ChoiceRow {
            applicable: Qt.platform.os === "osx"
            title: qsTr("Spatial audio")
            description: qsTr("Surround sound is rendered to spatial audio when playing through headphones, built-in speakers, or any 2-channel output device.")
            controlEnabled: StreamingPreferences.audioConfig !== StreamingPreferences.AC_STEREO
            selectedValue: StreamingPreferences.spatialAudioConfig
            onValueActivated: function(value) { StreamingPreferences.spatialAudioConfig = value }

            model: ListModel {
                ListElement { text: qsTr("Enabled"); val: StreamingPreferences.SAC_AUTO }
                ListElement { text: qsTr("Disabled"); val: StreamingPreferences.SAC_DISABLED }
            }
        }

        ToggleRow {
            applicable: Qt.platform.os === "osx"
            title: qsTr("Enable head tracking")
            description: qsTr("Requires supported Apple or Beats headphones. May cause audio glitches.")
            controlEnabled: StreamingPreferences.audioConfig !== StreamingPreferences.AC_STEREO &&
            ...
            onToggled: function(value) { StreamingPreferences.spatialHeadTracking = value }
        }
