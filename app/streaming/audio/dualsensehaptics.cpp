...
public:
    ...
    bool open() override
    {
        ...
        const bool deviceSet =
            getDeviceProperty(device, kAudioDevicePropertyDeviceUID, kAudioObjectPropertyScopeGlobal, uid) &&
            uid != nullptr &&
            AudioQueueSetProperty(m_Queue, kAudioQueueProperty_CurrentDevice, &uid, sizeof(uid)) == noErr;
        if (uid != nullptr) {
            CFRelease(uid);
        }
        if (!deviceSet) {
        ...
        for (UInt32 i = 0; i < QueueBufferCount; i++) {
            ...
            if (AudioQueueAllocateBuffer(m_Queue, QueueBufferFrames * EndpointBytesPerFrame, &buffer) != noErr) {
                close();
                return false;
            }
    ...
    WriteResult tryWrite(const Packet& packet) override
    {
        if (m_Queue == nullptr || packet.frameCount == 0) return WriteResult::Ok;
        if (!m_Alive.load(std::memory_order_relaxed)) return WriteResult::Failed;
        if (packet.frameCount > QueueBufferFrames) return WriteResult::Failed;
