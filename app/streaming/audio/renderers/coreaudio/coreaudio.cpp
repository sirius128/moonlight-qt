                                      ...
                                      AudioBufferList *ioData)
{
    ...
    for (uint32_t i = 0; i < spatialBuffer->mNumberBuffers; i++) {
        // faster version of memcpy(ioData->mBuffers[i].mData, spatialBuffer->mBuffers[i].mData, inNumberFrames * sizeof(float));
        // NB: vDSP_mmov() counts elements, not bytes
        vDSP_mmov((const float *)spatialBuffer->mBuffers[i].mData, (float *)ioData->mBuffers[i].mData, 1, inNumberFrames, 1, 1);
    }
...
bool CoreAudioRenderer::prepareForPlayback(const OPUS_MULTISTREAM_CONFIGURATION* opusConfig)
{
    ...
    if (opusConfig->channelCount > 2) {
        // Spatialize whenever we're not feeding real surround speakers, and also when
        // the output device simply has fewer channels than the stream carries (a 2-channel
        // USB DAC reports itself as external speakers, but can't take 6/8/12 channels).
    ...
    if (m_Spatial) {
        ...
        // this callback is non-interleaved stereo
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    ...
                    opusConfig->channelCount, outputType, m_OutputASBD.mChannelsPerFrame);

        if (!m_SpatialAU.setup(outputType, opusConfig->sampleRate, opusConfig->channelCount)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "CoreAudioRenderer failed to set up the spatial mixer for %d channels",
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "CoreAudioRenderer is using passthrough mode (%d-channel stream, output type %d, device has %u channels)",
                    opusConfig->channelCount, outputType, m_OutputASBD.mChannelsPerFrame);
    }
