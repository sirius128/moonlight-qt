                       ...
                       AudioBufferList *ioData)
{
    ...
    for (uint32_t i = 0; i < ioData->mNumberBuffers; i++) {
        // faster version of memset((float *)ioData->mBuffers[i].mData, 0, inNumberFrames * sizeof(float));
        // NB: vDSP_vclr() counts elements, not bytes
    ...
    // de-interleave ringBuffer PCM data into per-channel buffers, reordering from
    // the host's channel order into the mixer's channel layout on the way
    const float zero = 0.0f;
    for (uint32_t channel = 0; channel < channelCount; channel++) {
        float *channelBuffer = (float *)ioData->mBuffers[channel].mData;
        vDSP_vsadd(ringBuffer + me->m_ChannelMap[channel], channelCount, &zero, channelBuffer, 1, inNumberFrames);
    }
...
// Sunshine and GFE send interleaved PCM in WAVE channel order:
// FL FR FC LFE BL BR SL SR (TFL TFR TBL TBR). kAudioChannelLayoutTag_WAVE_5_1_B and
// kAudioChannelLayoutTag_WAVE_7_1 match that exactly, but kAudioChannelLayoutTag_Atmos_7_1_4
// orders the surrounds the other way around - L R C LFE Ls Rs Rls Rrs Vhl Vhr Ltr Rtr -
// putting the side pair before the back pair. Without this remap, 7.1.4 streams play
// their side and back channels swapped.
void AUSpatialRenderer::buildChannelMap(int inChannelCount)
{
    SDL_assert(inChannelCount <= AUDIO_CONFIGURATION_MAX_CHANNEL_COUNT);
...
bool AUSpatialRenderer::setup(AUSpatialMixerOutputType outputType, float sampleRate, int inChannelCount)
{
    ...
    buildChannelMap(inChannelCount);

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "CoreAudioRenderer spatial mixer input: %d channels, layout tag 0x%x",
                inChannelCount, layout);
